...
| - | ---- | ----------- | -------------- |
| 1 | [discovery.md](./discovery.md) | `intake-synthesize` → `grill-me` → `grill-to-handoff` | Context inbox routing (CI-01 Slack note → OQ → decision); goals drafted early; R-01 auto-launched; grill capture with resolved decisions D1–D8 and honest OQs |
| 2 | [research/R-01-…](./research/R-01-save-collection-comparables.md) | `research-spike` | Verification section (one claim removed as unverified); proposals — **not** merged into requirements; EAR-01 epic-adjacent idea |
...
| 6 | [rules/feature.md](./rules/feature.md) | `increment-requirements` | Durable rules citing their source PDRs |
| 7 | [context-map.md](./context-map.md) | session end | Hot cache + do-not-drift list a new session reads first |
| 8 | [dev-handoff.md](./dev-handoff.md) + [spec.md](./spec.md) | `dev-handoff` | DoR verified; spec stub with product summary prefilled and `[ENG]` sections empty for engineering |
